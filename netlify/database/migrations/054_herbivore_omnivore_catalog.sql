...
-- Renumbered from 050 to avoid duplicate migration number.
UPDATE animal_catalog SET species='Сетчатый жираф', scientific_name='Giraffa reticulata', subspecies=ARRAY[]::text[] WHERE id='zoo-giraffe';
UPDATE animal_catalog SET species='Западная горилла', scientific_name='Gorilla gorilla', subspecies=ARRAY['западная равнинная горилла','кросс-риверская горилла']::text[] WHERE id='zoo-gorilla';
...
ON CONFLICT (id) DO UPDATE SET category=EXCLUDED.category,species=EXCLUDED.species,scientific_name=EXCLUDED.scientific_name,subspecies=EXCLUDED.subspecies,sort_order=EXCLUDED.sort_order;
UPDATE animal_catalog SET species='Красная речная свинья', scientific_name='Potamochoerus porcus' WHERE id='zoo-red-river-hog';
UPDATE animal_catalog SET species='Шимпанзе', scientific_name='Pan troglodytes' WHERE id='zoo-chimpanzee';
UPDATE animal_catalog SET species='Африканский страус', scientific_name='Struthio camelus' WHERE id='zoo-ostrich';
