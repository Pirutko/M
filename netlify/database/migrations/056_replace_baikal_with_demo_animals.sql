-- MOSTIK v5.4 — replace the Baikal demo with five fresh demo animals.
-- Idempotent: removes only the old demo Baikal records and recreates/refreshes demo-* records.
BEGIN;

-- Remove Baikal-dependent demo records in FK-safe order.
...
ON CONFLICT (id) DO UPDATE SET
  name=EXCLUDED.name, species=EXCLUDED.species, breed=EXCLUDED.breed, owner_id=EXCLUDED.owner_id;

-- Shared demo access for role testing.
INSERT INTO animal_access(user_id,animal_id) VALUES
('demo-trainer','demo-animal-archie'),('demo-trainer','demo-animal-nora'),('demo-trainer','demo-animal-filya'),
...
ON CONFLICT (id) DO UPDATE SET
  animal_id=EXCLUDED.animal_id,name=EXCLUDED.name,signal=EXCLUDED.signal,goal=EXCLUDED.goal;
...
-- Three training sessions per animal for visible analytics.
...
SELECT 'demo-new-session-'||a.code||'-'||n,
       a.id,
       CASE WHEN n%2=0 THEN 'demo-trainer2' ELSE 'demo-trainer' END,
       now() - ((n*8 + a.ord)||' days')::interval,
       now() - ((n*8 + a.ord)||' days')::interval + interval '25 minutes',
       25,
       'успешно','',
       6 + n,
       CASE WHEN n=1 THEN 'низкий' WHEN n=2 THEN 'умеренный' ELSE 'низкий' END,
       'Обычная обстановка',
       CASE WHEN n=3 THEN 'средний' ELSE 'низкий' END,
       'Хорошая мотивация',
       3+n%3, 2+n%2
FROM (VALUES
...
SELECT 'demo-new-session-'||a.code||'-'||n,
       CASE WHEN n%2=1 THEN a.skill1 ELSE a.skill2 END,
       5+n
FROM (VALUES
...
-- Four observations per animal with a simple improving trend.
...
SELECT 'demo-new-obs-'||a.code||'-'||n,
       a.id,
       CASE WHEN n%3=0 THEN 'demo-keeper' WHEN n%3=1 THEN 'demo-owner' ELSE 'demo-trainer' END,
       now() - ((n*7 + a.ord)||' days')::interval,
       CASE WHEN n<3 THEN 'Спокойный, исследует окружение, постепенно привыкает к новой задаче' ELSE 'Уверенно взаимодействует с человеком и быстрее возвращает внимание к задаче' END,
       'Без заметных изменений.',
       2+n%3, 1+(n%3), 2+n, 3+n%2, 1, 3+n%3,
...
-- One active homework per animal.
INSERT INTO homework(id,animal_id,trainer_id,skill_id,title,instructions,due_date,status)
VALUES
('demo-new-hw-archie','demo-animal-archie','demo-trainer','demo-skill-archie-sit','Сидеть дома','3 подхода по 5 повторений. Завершать на успешном повторе.',current_date+3,'active'),
('demo-new-hw-nora','demo-animal-nora','demo-trainer','demo-skill-nora-carrier','Спокойная переноска','Оставлять переноску открытой и поощрять добровольный подход.',current_date+5,'active'),
('demo-new-hw-filya','demo-animal-filya','demo-trainer','demo-skill-filya-hand','Подход к руке','Короткие подходы, поощрять добровольное движение к ладони.',current_date+4,'active'),
...
('demo-new-hw-sonya','demo-animal-sonya','demo-trainer','demo-skill-sonya-touch','Спокойный контакт','Короткие касания без удержания, завершать до появления напряжения.',current_date+6,'active')
ON CONFLICT (id) DO NOTHING;

-- One veterinary record per animal.
...
-- Active diets and meal templates.
INSERT INTO diets(id,animal_id,name,description,active,target_calories,created_by) VALUES
('demo-diet-archie','demo-animal-archie','Рацион Арчи','Демонстрационный рацион собаки: основные приёмы пищи и тренировочное лакомство.',true,1450,'demo-keeper'),
('demo-diet-nora','demo-animal-nora','Рацион Норы','Демонстрационный рацион кошки с двумя основными приёмами пищи.',true,280,'demo-keeper'),
('demo-diet-filya','demo-animal-filya','Рацион Фили','Сено как основа, зелень и небольшая добавка гранул.',true,220,'demo-keeper'),
('demo-diet-rocky','demo-animal-rocky','Рацион Рокки','Демонстрационный рацион кореллы с зерновой смесью и овощами.',true,90,'demo-keeper'),
('demo-diet-sonya','demo-animal-sonya','Рацион Сони','Демонстрационный рацион черепахи с зеленью и овощами.',true,120,'demo-keeper')
ON CONFLICT (id) DO UPDATE SET
  name=EXCLUDED.name,description=EXCLUDED.description,active=true,target_calories=EXCLUDED.target_calories,updated_at=now();
...
-- Recent food logs for each animal.
INSERT INTO food_logs(id,animal_id,author_id,logged_at,meal,offered,eaten,not_eaten,appetite,note)
SELECT 'demo-new-food-'||a.code||'-'||n,
       a.id,'demo-keeper',now() - ((n+a.ord)||' days')::interval,
